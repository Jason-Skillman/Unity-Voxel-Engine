namespace VoxelEngine.Utilities
{
	...
	{
		public const int CubeMaxVertices8 = 8;
		...
		public static readonly float3[] CubeVertices8 =
		{
		...
		public static void BuildCube_24Verts(
			//Output
			ref VertexDataPositionOnly vertexData,
			ref int vertexIndex,
			//Input
			in Block block,
			IncludeFaces includeFaces = IncludeFaces.All)
		...
		[BurstCompile]
		public static void BuildUV0(
			//Output
			ref UnsafeList<float2> uv0,
			//Input
			int vertexIndex = 0,
			IncludeFaces includeFaces = IncludeFaces.All)
		{
			...
			{
				const int Offset = 0; 
				uv0[vertexIndex + Offset]     = new float2(0, 0);	//Bottom left
				uv0[vertexIndex + Offset + 1] = new float2(1, 0);	//Bottom right
				uv0[vertexIndex + Offset + 2] = new float2(1, 1);	//Top right
				uv0[vertexIndex + Offset + 3] = new float2(0, 1);	//Top left
			}
			if((includeFaces & IncludeFaces.Back) == IncludeFaces.Back)
			{
				const int Offset = 4; 
