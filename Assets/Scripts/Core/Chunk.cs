namespace VoxelEngine.Core
{
	...
	public struct Chunk : IDisposable
	{
		...
		/// <summary>
		/// Contains all the collider vertex data of all blocks in this chunk.
		/// </summary>
		public VertexDataPositionOnly colliderVertexData;

		public int2 LocalPosition => localPosition;
		public Allocator Allocator => allocator;
		public int2 GetWorldPosition => LocalToWorldPosition(LocalPosition);
		...
		public Chunk(int2 localPosition, Allocator allocator)
		{
			...
			colliderVertexData = new VertexDataPositionOnly(allocator, MaxVertices24, MaxIndices);
		...
		[Pure]
		public bool CheckAdjacentBlockIsSolid(Direction direction, in int3 blockPosition, in ChunkWorld chunkWorld)
		{
			int3 directionOffset = DirectionUtils.GetDirection(direction);
			int3 adjacentBlockPosition = directionOffset + blockPosition;

			//Check if the adjacent block position is within the chunk.
			if(IsValidPosition(adjacentBlockPosition))
			{
				Block adjacentBlock = GetBlock(adjacentBlockPosition);
				return adjacentBlock.IsSolid();
			}
			...
			{
				int2 worldPos = LocalToWorldPosition(localPosition);
				
				int2 worldPosition = new(worldPos.x + adjacentBlockPosition.x, worldPos.y + adjacentBlockPosition.z);
				int chunkX = (int)math.floor(worldPosition.x / (float)Width);
				int chunkY = (int)math.floor(worldPosition.y / (float)Length);
				int2 chunkPosition = new(chunkX, chunkY);

				if (ChunkWorld.IsValidPosition(worldPosition))
				{
					Chunk adjacentChunk = chunkWorld.GetChunk(chunkPosition.x, chunkPosition.y);
					int2 localPos = new(worldPosition.x % Width, worldPosition.y % Length);
					Block adjacentBlock = adjacentChunk.GetBlock(new int3(localPos.x, adjacentBlockPosition.y, localPos.y));
					return adjacentBlock.blockType != BlockType.Empty;
				}
				return false;
			}
		}
